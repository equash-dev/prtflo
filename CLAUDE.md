...
**Content flows from `config/`, never from component code.** `config/products.ts` (catalogue + `PRODUCT_MAP` + `productsByCategory`), `config/categories.ts`, `config/site.ts`, `config/copy.ts`, `config/currencies.ts` — all typed by `types/`. The catalogue is the 20-piece SS26 range transcribed from `PRTFLO_linesheet_v1.xlsx` (source of truth for codes, colourways, fabrics, descriptions, and the four-shot manifest: 01 ghost hero, 02 editorial, 03 mid-crop, 04 detail macro). To add a product: append to `PRODUCTS` (keep `scripts/ingest-assets.mjs`'s code→slug map in sync) and run `npm run ingest` on its PNGs. Category slugs are `men` / `women` / `home` (`types/product.ts`); home is intentionally empty until its range lands.
