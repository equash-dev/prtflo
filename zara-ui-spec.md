...
## Global tokens & feel
- **Font:** Helvetica Now Text, **weight 300 dominant** (light). PRTFLO keeps **Geist** (neo-grotesque
...
- **Tracking:** essentially **normal (0)** everywhere — product titles, nav, prices, micro-labels.
  This is the biggest departure from PRTFLO's current 0.18–0.28em tracking. **Reduce tracking hard.**
  Keep a light tracking (~0.1em) only on the PRTFLO wordmark/serial as a brand signature.
- **Colour mapping (keep all 7 PRTFLO warm tokens):** Zara white `#fff` → PRTFLO `canvas`; Zara product
  grey panel `~#ECECEC` → PRTFLO `panel`; Zara black text `#000` → PRTFLO `ink`; greys → `muted`/`hairline`.
- **Type scale (measured):**
...
- **Buttons:** **square — `border-radius: 0`**, 1px hairline border, white/transparent fill,
  uppercase 13px/300, ~40–48px tall. (PRTFLO's pill buttons become squared for commerce CTAs.)
...
- **PRTFLO adaptation:** keep wordmark + serial (small, left, beside/below hamburger). Drawer lists
...
- **PRTFLO adaptation:** drop `shortDescription` from card (name + price only, Zara-style). Keep
...
- **PRTFLO adaptation:** full-bleed hero (warm `panel` placeholders since no photos), large PRTFLO wordmark,
  then **category entry blocks** (MEN / WOMEN / ARCHIVE as big image tiles), then the reframed ROI
  strip, and the reveal-line → `/intro` link. Keep all PRTFLO copy.
...
- **PRTFLO adaptation:** accordion columns (Collections / House / Contact), newsletter, region+currency,
  legal row. Keep reveal-line → `/intro` link and © line.

## Decisions / tensions logged
- **Square buttons** override PRTFLO's pill — required for Zara structural match; applies to commerce
  CTAs. (PRTFLO `Button` gains a squared default or a `shape` prop.)
- **Tight tracking + light weight** override PRTFLO's wide-tracked medium type, except the wordmark.
